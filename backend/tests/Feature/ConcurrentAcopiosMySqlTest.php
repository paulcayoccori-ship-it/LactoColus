...
PHP;
    ...
    public function test_closing_the_same_journey_concurrently_does_not_duplicate_no_entrego_rows(): void
    {
        if (getenv('LACTOCOLUS_MYSQL_TESTS') !== '1') {
            $this->markTestSkipped('Requiere un MySQL exclusivo; ejecutar con LACTOCOLUS_MYSQL_TESTS=1.');
        }
        $database = 'lactocolus_test_cierre_'.bin2hex(random_bytes(8));
        $serverConfig = array_replace(config('database.connections.mysql'), ['database' => null, 'url' => null]);
        config(['database.connections.acopio_server' => $serverConfig]);
        $server = DB::connection('acopio_server');
        $created = false;
        ...
        try {
            ...
            $collector->assignRole('recolector');
            $route = RutaAcopio::factory()->create(['recolector_id' => $collector->id]);
            $producers = Productor::factory()->count(3)->create();
            ...
            $journey = app(GestionarAcopios::class)->createJourney($collector->id, ['ruta_id' => $route->id, 'fecha_operativa' => '2026-09-09', 'turno' => 'primera_vuelta'], true);
            $worker = <<<'PHP'
require 'vendor/autoload.php'; $app=require 'bootstrap/app.php'; $app->make(Illuminate\Contracts\Console\Kernel::class)->bootstrap(); config(['database.default'=>'mysql','database.connections.mysql.database'=>$argv[1],'database.connections.mysql.url'=>null]); Illuminate\Support\Facades\DB::purge('mysql'); Illuminate\Support\Facades\Auth::guard('web')->setUser(App\Models\User::query()->findOrFail((int)$argv[2])); try { $r=app(App\Application\Acopios\GestionarAcopios::class)->closeJourneyForCollector((int)$argv[2], $argv[3]); echo json_encode($r); } catch (Illuminate\Validation\ValidationException $e) { echo 'error:'.json_encode($e->errors()); exit(1); } catch (Throwable $e) { echo 'error:'.$e->getMessage(); exit(1); }
PHP;
            foreach (range(1, 2) as $i) {
                $process = new Process([PHP_BINARY, '-r', $worker, $database, (string) $collector->id, $journey['uuid_publico']], base_path(), ['APP_ENV' => 'testing'], timeout: 20);
                $process->start();
                $processes[] = $process;
            }
            $outputs = [];
            foreach ($processes as $process) {
                $outputs[] = ['code' => $process->wait(), 'out' => $process->getOutput(), 'err' => $process->getErrorOutput()];
            }
            $successes = array_filter($outputs, fn (array $o) => $o['code'] === 0);
            $failures = array_filter($outputs, fn (array $o) => $o['code'] !== 0);
            $this->assertCount(1, $successes, implode(' | ', array_column($outputs, 'out')));
            $this->assertCount(1, $failures);
            $this->assertStringContainsString('cerrada o anulada', $failures[array_key_first($failures)]['out']);
            $this->assertDatabaseCount('entregas_acopio', 3);
            $this->assertSame(3, DB::table('entregas_acopio')->where('jornada_id', $journey['id'])->where('no_entrego', true)->count());
            $this->assertSame(1, DB::table('auditorias_acopio')->where('jornada_id', $journey['id'])->where('accion', 'cierre')->count());
        } finally {
            foreach ($processes as $process) {
                $process->stop();
            }
